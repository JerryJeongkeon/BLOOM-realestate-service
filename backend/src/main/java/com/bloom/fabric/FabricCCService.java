...
import com.bloom.service.impl.BuildingService;
...
public class FabricCCService implements IFabricCCService {
	...
	/**
	 * 2주차 과제2: 채널 접근 체인코드를 이용하기 위하여 구축해놓은 패브릭 네트워크의 채널을 가져오는 기능을 구현한다.
	 */
	...
	private MaintenanceRecord getMaintenanceRecord(JsonObject object) {
		MaintenanceRecord maintenanceRecord = new MaintenanceRecord(object.getString("num"),
				object.getString("address"), object.getString("dong"), object.getString("ho"),
				object.getString("latitude"), object.getString("longitude"), object.getString("category"),
				object.getString("details"), object.getString("cost"), object.getString("startDate"), 
				object.getString("endDate"), object.getString("license"), object.getString("image"),
				object.getString("createdAt"), object.getString("expiredAt"));
	...
	public FabricRecord query(String num) {
		if (this.hfClient == null || this.channel == null) {
			logger.info("조회 채널 로드하겠습니다.");
			loadChannel();
			logger.info("조회 채널 로드했슴다.");
		}
...
//    @Override
//    public boolean registerBuildingInfo(String num, String address, String dong, String ho, String latitude, String longitude, String supply, String exclusive, String details, String cost, String startDate, String endDate, String name, String license, String image, String createdAt, String expiredAt) {
//        if (this.hfClient == null || this.channel == null) {
//            logger.info("등록 채널 로드하겠습니다.");
//            loadChannel();
//            logger.info("등록 채널 로드했슴다.");
//        }
//        ChaincodeID id = ChaincodeID.newBuilder().setName("bloom06").build();
...
//        String[] args = { num, address, dong, ho, latitude, longitude, supply, exclusive, details, cost, startDate,
//                endDate, name, license, image, createdAt, expiredAt };
//        tpr.setArgs(args);
//        Collection<ProposalResponse> res;
//        try {
...
//    }
	...
	public List<FabricRecord> getBuildingHistory(String Serial) {
		if (this.hfClient == null || this.channel == null) {
			logger.info("빌딩이력 채널 로드하겠습니다.");
			loadChannel();
			logger.info("빌딩이력 채널 로드했슴다.");
		}
		ChaincodeID id = ChaincodeID.newBuilder().setName("bloom20").build();
		QueryByChaincodeRequest qpr = hfClient.newQueryProposalRequest();
		qpr.setChaincodeID(id);
	...
	public MaintenanceRecord queryMaintenance(String num) {
		...
		try {
			...
			for (ProposalResponse response : res) {
				...
				JsonReader parser = Json
						.createReader(new ByteArrayInputStream(response.getChaincodeActionResponsePayload()));
				maintenanceRecord = getMaintenanceRecord(parser.readObject());
			}
			return maintenanceRecord;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public boolean registerMaintenanceInfo(MaintenanceRecord mr) {
		...
		logger.info("Request Regist Building Date :" + mr.toString());

		ChaincodeID id = ChaincodeID.newBuilder().setName("bloom20").build();
		TransactionProposalRequest tpr = hfClient.newTransactionProposalRequest();

		tpr.setChaincodeID(id);
		tpr.setFcn("registerMaintenanceInfo");
		String[] args = { mr.getNum(), mr.getAddress(), mr.getDong(), mr.getHo(), mr.getLatitude(), mr.getLongitude(),
				mr.getCategory(), mr.getDetails(), mr.getCost(), mr.getLicense(), mr.getImage(), mr.getStartDate(),
				mr.getEndDate(), mr.getCreatedAt(), mr.getExpiredAt() };
		tpr.setArgs(args);
		Collection<ProposalResponse> res;
		try {
